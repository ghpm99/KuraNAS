...
describe('PlaylistsProvider', () => {
    ...
    it('creates a playlist and shows success snackbar', async () => {
        ...
        await waitFor(() =>
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_PLAYLIST_CREATED', {
                variant: 'success',
            })
        );
        expect(result.current.createOpen).toBe(false);
    ...
    it('shows error snackbar on delete failure', async () => {
        ...
        await waitFor(() =>
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_PLAYLIST_DELETE_FAILED', {
                variant: 'error',
            })
        );
    });
    ...
    it('removes a track when a playlist is selected and shows success snackbar', async () => {
        ...
        await waitFor(() =>
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_TRACK_REMOVED', {
                variant: 'success',
            })
        );
    });
    ...
    it('shows error snackbar on remove track failure', async () => {
        ...
        await waitFor(() =>
            expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_TRACK_REMOVE_FAILED', {
                variant: 'error',
            })
        );
    });
