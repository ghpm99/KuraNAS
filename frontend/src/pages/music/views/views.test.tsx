...
const clickBackButton = (container: HTMLElement) => {
    fireEvent.click(
        container.querySelector('svg.lucide-arrow-left')?.closest('button') as HTMLElement
    );
};
...
beforeEach(() => {
    ...
    mockGetMusicGenres.mockResolvedValue(
        makePagination([{ key: 'genre-1', genre: 'genre-1', track_count: 1 }])
    );
    mockGetMusicByGenre.mockResolvedValue(makePagination([track]));
    mockGetMusicFolders.mockResolvedValue(
        makePagination([{ folder: '/root/folder', track_count: 1 }])
    );
    mockGetMusicByFolder.mockResolvedValue(
        makePagination([
            track,
            { ...track, id: 2, path: '/other/track-2.mp3', parent_path: '/other' },
        ])
    );
    ...
    mockUseInfiniteQuery.mockImplementation((options: any) => {
        ...
        switch (key) {
            ...
            case 'music-folders':
                return makeInfiniteResult(
                    [
                        { folder: '/root/folder', track_count: 1 },
                        { folder: '/', track_count: 2 },
                    ],
                    true
                );
...
describe('music views', () => {
    ...
    it('renders artists list/detail flow, load-more and back', async () => {
        ...
        mockUseInfiniteQuery.mockImplementation((options: any) => {
            ...
            if (key === 'music-by-artist') {
                return {
                    ...makeInfiniteResult([track], true),
                    fetchNextPage: fetchArtistTracks,
                };
            }
            return makeInfiniteResult([]);
        });
    ...
    it('renders genres flow, detail load-more and menu click', async () => {
        ...
        mockUseInfiniteQuery.mockImplementation((options: any) => {
            ...
            if (key === 'music-by-genre')
                return {
                    ...makeInfiniteResult([track], true),
                    fetchNextPage: fetchGenreTracks,
                };
            return makeInfiniteResult([]);
        });
            ...
            if (key === 'music-genres') {
                return {
                    data: {
                        pages: [
                            makePagination(
                                [{ key: 'genre-1', genre: 'genre-1', track_count: 1 }],
                                true
                            ),
                        ],
                    },
    ...
    it('renders load-more text/actions for albums and artists and opens folder add-menu', () => {
        ...
        mockUseInfiniteQuery.mockImplementation((options: any) => {
            ...
            if (key === 'music-folders')
                return makeInfiniteResult([{ folder: '/', track_count: 1 }]);
    ...
    it('covers genre keyboard navigation and play overlay with empty responses', async () => {
        ...
        const secondGenreCard = screen
            .getByText('genre-1')
            .closest('[role="button"]') as HTMLElement;
        fireEvent.keyDown(secondGenreCard, { key: 'Escape' });
        expect(
            secondRender.container.querySelector('svg.lucide-arrow-left')
