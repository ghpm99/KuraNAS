				...
				<div className={styles.span6}>
					...
					>
						<Box className={styles.inlineSummary}>
							...
							<LinearProgress variant='determinate' value={Math.min(100, usedPercent)} className={styles.progress} />
						</Box>
						<Table size='small'>
							<TableHead>
								<TableRow>
						...
						<Table size='small'>
							...
							<TableBody>
								{data?.top_folders.map((folder) => (
									<TableRow key={folder.path} hover onClick={() => navigate(appRoutes.files)} className={styles.tableCellAction}>
