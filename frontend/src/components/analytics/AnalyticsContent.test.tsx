...
jest.mock('@/components/i18n/provider/i18nContext', () => ({
	__esModule: true,
	default: () => ({ t: (key: string, values?: Record<string, string>) => (values?.minutes ? `${key}:${values.minutes}` : key) }),
}));
	...
	beforeEach(() => {
		...
		mockUseAnalyticsOverview.mockReturnValue({
			...
			data: {
				...
				hot_folders: [{ path: '/media', new_files: 2, added_bytes: 10, last_event: '2026-01-01T00:00:00Z' }],
				top_folders: [{ path: '/media/videos', files: 1, bytes: 1024, last_modified: '2026-01-01T00:00:00Z' }],
				recent_files: [
					{
						id: 1,
						name: 'movie.mp4',
				...
				duplicates: {
					...
					top_groups: [{ signature: 'abcdef1234567890', copies: 2, size_bytes: 128, reclaimable_size: 128, paths: ['/media/videos/movie.mp4'] }],
				},
				library: {
					categorized_media: 8,
