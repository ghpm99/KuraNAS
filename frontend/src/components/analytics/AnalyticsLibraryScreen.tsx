						...
						<Table size='small'>
							<TableHead>
								<TableRow>
									...
									<TableCell align='right'>{t('ANALYTICS_ITEMS_INDEXED')}</TableCell>
								</TableRow>
							</TableHead>
							<TableBody>
								<TableRow>
									<TableCell>{t('NAV_MUSIC')}</TableCell>
									<TableCell align='right'>{(data?.library.audio_with_metadata ?? 0).toLocaleString()}</TableCell>
								</TableRow>
								<TableRow>
									<TableCell>{t('NAV_VIDEOS')}</TableCell>
									<TableCell align='right'>{(data?.library.video_with_metadata ?? 0).toLocaleString()}</TableCell>
								</TableRow>
								<TableRow>
									<TableCell>{t('NAV_IMAGES')}</TableCell>
									<TableCell align='right'>{(data?.library.image_with_metadata ?? 0).toLocaleString()}</TableCell>
								</TableRow>
								<TableRow>
									<TableCell>{t('ANALYTICS_IMAGES_CLASSIFIED')}</TableCell>
									<TableCell align='right'>{(data?.library.image_classified ?? 0).toLocaleString()}</TableCell>
								</TableRow>
							</TableBody>
						</Table>
					</AnalyticsSection>
						...
						<Table size='small'>
							...
							<TableBody>
								<TableRow>
									...
									<TableCell align='right'>{(data?.processing.metadata_pending ?? 0).toLocaleString()}</TableCell>
									<TableCell align='right'>{(data?.processing.metadata_failed ?? 0).toLocaleString()}</TableCell>
								</TableRow>
								<TableRow>
									<TableCell>{t('ANALYTICS_THUMBNAIL_LABEL')}</TableCell>
									<TableCell align='right'>{(data?.processing.thumbnail_pending ?? 0).toLocaleString()}</TableCell>
									<TableCell align='right'>{(data?.processing.thumbnail_failed ?? 0).toLocaleString()}</TableCell>
								</TableRow>
							</TableBody>
						</Table>
					</AnalyticsSection>
						...
						<Table size='small'>
							...
							<TableBody>
								{data?.recent_files.slice(0, 10).map((file) => (
									<TableRow key={file.id} hover onClick={() => navigate(appRoutes.files)} className={styles.tableCellAction}>
