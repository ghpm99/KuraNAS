...
describe('useImageContent', () => {
	...
	it('returns captures section and filters to capture category', () => {
		mockUseImage.mockReturnValue({
			...defaultImageReturn(),
			images: [
				createImage({ id: 1, metadata: { classification: { category: 'capture', confidence: 0.98 } } }),
				createImage({ id: 2, name: 'photo.jpg', metadata: { classification: { category: 'photo', confidence: 0.9 } } }),
			],
		});
	...
	it('groups images by date with known date', () => {
		...
		expect(result.current.groupByLabels).toEqual({ date: 'Data', type: 'Tipo', name: 'Nome' });
	});
	...
	it('returns album selection title and description when album is selected', () => {
		...
		expect(result.current.activeSelectionDescription).toBe('Tudo que nao entrou nos temas principais');
	...
	it('returns coverImageId in album cards', () => {
		mockUseImage.mockReturnValue({
			...defaultImageReturn(),
			images: [createImage({ id: 5, metadata: { width: 800, height: 600, classification: { category: 'other', confidence: 0.5 } } })],
		});
	...
	it('shows success snackbar when favorite is removed', async () => {
		...
		await waitFor(() => expect(mockEnqueueSnackbar).toHaveBeenCalledWith(
