...
describe('imageLibraryData', () => {
	describe('getImageDate', () => {
		...
		it('falls back to createdAt when datetime fields are empty', () => {
			const date = getImageDate(createImage({
				metadata: { datetime_original: '', datetime: '', createdAt: '2026-01-20T10:00:00Z' },
			}));
		...
		it('skips invalid date strings', () => {
			const date = getImageDate(createImage({
				...
				created_at: '2025-12-25T10:00:00Z',
				metadata: { datetime_original: 'invalid', datetime: 'also-invalid', createdAt: '' },
			}));
			expect(date).toEqual(new Date('2025-12-25T10:00:00Z'));
		});
	});
	...
	describe('hasPersistedCategory', () => {
		...
		it('returns false when classification is undefined', () => {
			const image = createImage({ metadata: { classification: undefined as unknown as { category: 'photo'; confidence: number } } });
			expect(hasPersistedCategory(image, 'photo')).toBe(false);
		});
	});
	...
	describe('getImageDirectoryPath', () => {
		...
		it('strips filename from path when name matches end of path', () => {
			expect(getImageDirectoryPath(createImage({ name: 'image.jpg', path: '/library/trips/image.jpg' }))).toBe('/library/trips');
		});

		it('returns / when path is just the filename', () => {
			expect(getImageDirectoryPath(createImage({ name: 'image.jpg', path: 'image.jpg' }))).toBe('/');
		});
		...
		it('strips file-like last segment when name does not match', () => {
			expect(getImageDirectoryPath(createImage({ name: 'different.jpg', path: '/library/trips/photo.jpg' }))).toBe('/library/trips');
		});

		it('returns full path when last segment is a directory (no dot)', () => {
			expect(getImageDirectoryPath(createImage({ name: 'different.jpg', path: '/library/camera' }))).toBe('/library/camera');
		});

		it('normalizes backslashes', () => {
			expect(getImageDirectoryPath(createImage({ name: 'image.jpg', path: 'C:\\library\\trips\\image.jpg' }))).toBe('C:/library/trips');
		});

		it('handles path with no name set', () => {
			expect(getImageDirectoryPath(createImage({ name: '', path: '/library/trips/image.jpg' }))).toBe('/library/trips');
		});
	});
	...
	describe('getAutomaticAlbumKey', () => {
		it('classifies meme images', () => {
			expect(getAutomaticAlbumKey(createImage({ name: 'funny-meme.jpg', path: '/memes/funny-meme.jpg' }))).toBe('memes');
		});

		it('classifies document images', () => {
			expect(getAutomaticAlbumKey(createImage({ name: 'receipt.jpg', path: '/docs/receipt.jpg', metadata: { width: 900, height: 1400 } }))).toBe('documents');
		});

		it('classifies travel images when path matches and category is photo', () => {
			expect(getAutomaticAlbumKey(createImage({ name: 'beach-day.jpg', path: '/travel/beach-day.jpg' }))).toBe('travel');
		});

		it('does not classify as travel when category is not photo', () => {
			expect(getAutomaticAlbumKey(createImage({
				name: 'beach-day.jpg',
				path: '/travel/beach-day.jpg',
				metadata: { classification: { category: 'capture', confidence: 0.9 }, width: 800, height: 600 },
			}))).toBe('others');
		});
		...
		it('handles image with no metadata for album classification', () => {
			const image = { ...createImage({ name: 'generic.jpg', path: '/library/generic.jpg' }), metadata: undefined } as IImageData;
	...
	describe('buildFolderCollections', () => {
		...
		it('updates cover when a newer image is added to the folder', () => {
			const collections = buildFolderCollections([
				createImage({ id: 1, name: 'old.jpg', path: '/library/photos/old.jpg', metadata: { datetime_original: '2026-01-01T10:00:00Z' } }),
				createImage({ id: 2, name: 'new.jpg', path: '/library/photos/new.jpg', metadata: { datetime_original: '2026-03-15T10:00:00Z' } }),
			]);
		...
		it('does not update cover when added image is older', () => {
			const collections = buildFolderCollections([
				createImage({ id: 1, name: 'new.jpg', path: '/library/photos/new.jpg', metadata: { datetime_original: '2026-03-15T10:00:00Z' } }),
				createImage({ id: 2, name: 'old.jpg', path: '/library/photos/old.jpg', metadata: { datetime_original: '2026-01-01T10:00:00Z' } }),
			]);
		...
		it('sorts by id when dates are equal', () => {
			const collections = buildFolderCollections([
				createImage({ id: 1, name: 'a.jpg', path: '/aaa/a.jpg', metadata: { datetime_original: '2026-03-01T10:00:00Z' } }),
				createImage({ id: 2, name: 'b.jpg', path: '/bbb/b.jpg', metadata: { datetime_original: '2026-03-01T10:00:00Z' } }),
			]);
		...
		it('handles null latestDate in sorting', () => {
			const collections = buildFolderCollections([
				...
				createImage({ id: 2, name: 'dated.jpg', path: '/beta/dated.jpg', metadata: { datetime_original: '2026-03-01T10:00:00Z' } }),
			]);
	...
	describe('buildAutomaticAlbumCollections', () => {
		it('returns all five album buckets in order', () => {
			...
			expect(collections.map((c) => c.id)).toEqual(['travel', 'documents', 'wallpapers', 'memes', 'others']);
		...
		it('sets cover to first image added to album', () => {
			const collections = buildAutomaticAlbumCollections([
				createImage({ id: 1, name: 'misc1.jpg', path: '/library/random/misc1.jpg', metadata: { width: 800, height: 600 } }),
			]);
		...
		it('updates cover when newer image is added to same album', () => {
			const collections = buildAutomaticAlbumCollections([
				createImage({ id: 1, name: 'misc1.jpg', path: '/library/random/misc1.jpg', metadata: { width: 800, height: 600, datetime_original: '2026-01-01T10:00:00Z' } }),
				createImage({ id: 2, name: 'misc2.jpg', path: '/library/random/misc2.jpg', metadata: { width: 800, height: 600, datetime_original: '2026-03-15T10:00:00Z' } }),
			]);
		...
		it('updates cover when image has same date (>= comparison)', () => {
			const collections = buildAutomaticAlbumCollections([
				createImage({ id: 1, name: 'misc1.jpg', path: '/library/random/misc1.jpg', metadata: { width: 800, height: 600, datetime_original: '2026-03-01T10:00:00Z' } }),
				createImage({ id: 2, name: 'misc2.jpg', path: '/library/random/misc2.jpg', metadata: { width: 800, height: 600, datetime_original: '2026-03-01T10:00:00Z' } }),
			]);
		...
		it('handles images with null dates in albums', () => {
			const collections = buildAutomaticAlbumCollections([
				createImage({
					...
					updated_at: '',
					created_at: '',
					metadata: { width: 800, height: 600, datetime_original: '', datetime: '', createdAt: '' },
				}),
			]);
	...
	describe('matchesImageSearch', () => {
		...
		it('matches by name', () => {
			expect(matchesImageSearch(createImage({ name: 'Family dinner.jpg' }), 'family')).toBe(true);
		});

		it('matches by path', () => {
			expect(matchesImageSearch(createImage({ path: '/library/home/photo.jpg' }), 'home')).toBe(true);
		});
		...
		it('matches by make', () => {
			expect(matchesImageSearch(createImage({ metadata: { make: 'Canon' } }), 'canon')).toBe(true);
		});
		...
		it('matches by image_description', () => {
			expect(matchesImageSearch(createImage({ metadata: { image_description: 'Sunday dinner' } }), 'sunday')).toBe(true);
		});
		...
		it('handles image with no metadata', () => {
			const image = { ...createImage({ name: 'test.jpg' }), metadata: undefined } as IImageData;
	...
	describe('matchesImageSection', () => {
		...
		it('returns true for captures section with capture category', () => {
			const image = createImage({ metadata: { classification: { category: 'capture', confidence: 0.91 } } });
