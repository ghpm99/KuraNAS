		...
		<div className={styles.grid}>
			{cards.map((card) => (
				<button
					...
					className={card.id === selectedId ? `${styles.card} ${styles.cardActive}` : styles.card}
					aria-label={t('IMAGES_COLLECTION_OPEN', { name: card.title })}
				>
					<div className={styles.cover}>
						{card.coverImageId ? (
							<img src={thumbnailUrl(card.coverImageId)} alt={card.title} loading='lazy' />
						) : (
							<div className={styles.placeholder}>{card.title.slice(0, 1).toUpperCase()}</div>
						)}
					</div>
					<div className={styles.body}>
						...
						<span className={styles.count}>{t('IMAGES_PHOTOS_COUNT', { count: String(card.imageCount) })}</span>
					</div>
				</button>
			))}
		</div>
