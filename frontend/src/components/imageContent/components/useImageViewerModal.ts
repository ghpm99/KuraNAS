...
export const useImageViewerModal = ({
	...
	return useMemo(() => {
		...
		const classificationKey = classificationLabelKeys[activeImage.metadata?.classification?.category ?? 'other'];
		const resolution =
		...
		const deviceLabel = [activeImage.metadata?.make, activeImage.metadata?.model].filter(Boolean).join(' ');
		...
		const confidenceLabel =
			typeof confidenceValue === 'number' ? `${Math.round(confidenceValue * 100)}%` : t('COMMON_NOT_AVAILABLE');

		const details: ViewerDetailSection[] = [
			{
				title: t('IMAGES_DETAILS_SECTION_LIBRARY'),
				items: [
					{ label: t('IMAGES_DETAIL_FOLDER'), value: buildValue(folderPath, t('COMMON_NOT_AVAILABLE')) },
					{ label: t('IMAGES_DETAIL_FORMAT'), value: buildValue(activeImage.format, t('COMMON_NOT_AVAILABLE')) },
				...
				items: [
					{
						...
						value: activeImageDate ? dateFormatter.format(activeImageDate) : t('IMAGES_DATE_UNAVAILABLE'),
					},
					{
						label: t('IMAGES_DETAIL_CREATED'),
						value: activeImage.created_at ? dateFormatter.format(new Date(activeImage.created_at)) : t('COMMON_NOT_AVAILABLE'),
					},
					{
						label: t('IMAGES_DETAIL_SOFTWARE'),
						value: buildValue(activeImage.metadata?.software, t('COMMON_NOT_AVAILABLE')),
					},
					{
						label: t('IMAGES_DETAIL_DESCRIPTION'),
						value: buildValue(activeImage.metadata?.image_description, t('COMMON_NOT_AVAILABLE')),
					},
				],
			},
			{
				...
				items: [
					{ label: t('IMAGES_DETAIL_CAMERA'), value: buildValue(deviceLabel, t('COMMON_NOT_AVAILABLE')) },
					{
						label: t('IMAGES_DETAIL_LENS'),
						value: buildValue(activeImage.metadata?.lens_model, t('COMMON_NOT_AVAILABLE')),
					},
					{
						label: t('IMAGES_DETAIL_ISO'),
						value: formatNumberValue(activeImage.metadata?.iso) || t('COMMON_NOT_AVAILABLE'),
					},
					{
					...
					},
					{
						...
						value: formatExposureValue(activeImage.metadata?.exposure_time) || t('COMMON_NOT_AVAILABLE'),
					},
				],
			},
		];
