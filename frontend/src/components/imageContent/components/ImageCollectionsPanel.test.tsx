...
describe('ImageCollectionsPanel', () => {
	it('renders an empty state when there are no cards', () => {
		render(<ImageCollectionsPanel cards={[]} emptyTitle='Nada aqui' emptyDescription='Sem colecoes' onSelect={jest.fn()} />);
	...
	it('renders cards with placeholder/thumbnail and handles selection', () => {
		...
		expect(screen.getByAltText('Folder B')).toHaveAttribute('src', '/api/v1/files/thumbnail/10?width=960&height=720');
