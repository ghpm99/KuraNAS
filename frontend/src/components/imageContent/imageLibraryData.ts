...
const pathKeywordGroups = {
	travel: ['travel', 'trip', 'vacation', 'holiday', 'journey', 'viagem', 'ferias', 'praia', 'beach', 'camp', 'trail'],
	documents: ['scan', 'document', 'receipt', 'invoice', 'contract', 'doc', 'boleto', 'nota', 'comprovante'],
...
export const getImageDirectoryPath = (image: IImageData) => {
	...
	if (normalizedName && (normalizedPath === normalizedName || normalizedPath.endsWith(`/${normalizedName}`))) {
...
const matchesKeywords = (value: string, keywords: readonly string[]) => keywords.some((keyword) => value.includes(keyword));
...
export const buildAutomaticAlbumCollections = (images: IImageData[]) => {
	const albumOrder: AutomaticAlbumKey[] = ['travel', 'documents', 'wallpapers', 'memes', 'others'];
	...
	for (const image of images) {
		...
		if (!collection.cover || (imageDate?.getTime() ?? 0) >= (collection.latestDate?.getTime() ?? 0)) {
...
export const matchesImageSection = (section: ImageSection, image: IImageData, date: Date | null) => {
	switch (section) {
		case 'recent':
