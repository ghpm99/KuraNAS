...
function SummaryItem({ title, value, subtitle }: { title: string; value: ReactNode; subtitle?: string }) {
	return (
		<Box>
		...
		<Card title={t('DAY_SUMMARY_TITLE')}>
			<Grid container spacing={3}>
				<Grid size={{ xs: 12, sm: 4 }}>
					<SummaryItem title={t('TOTAL_ACTIVITIES')} value={data?.summary?.total_activities} />
				</Grid>
				<Grid size={{ xs: 12, sm: 4 }}>
					<SummaryItem
