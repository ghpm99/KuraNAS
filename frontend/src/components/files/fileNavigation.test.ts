...
describe('findTrailById', () => {
	const childNode = makeFile({ id: 3, name: 'child', path: '/root/parent/child', parent_path: '/root/parent' });
	const parentNode = makeFile({ id: 2, name: 'parent', path: '/root/parent', parent_path: '/root', file_children: [childNode] });
	const rootNode = makeFile({ id: 1, name: 'root', path: '/root', file_children: [parentNode] });
