...
const baseApi = () => ({
	...
	playbackContext: undefined as undefined | { labelKey: string; labelParams: Record<string, string> },
	toggleQueue: jest.fn(),
	queueOpen: false,
});
...
describe('GlobalPlayerControl', () => {
	beforeEach(() => {
		jest.clearAllMocks();
		mockGetMusicTitle.mockImplementation((track: { name: string; metadata?: { title?: string } }) => track.metadata?.title || track.name);
		mockGetMusicArtist.mockImplementation((track: { metadata?: { artist?: string } }) => track.metadata?.artist || 'Unknown Artist');
	});
	...
	it('shows playback context label when playbackContext exists', () => {
		mockUseGlobalMusic.mockReturnValue({
			...baseApi(),
			playbackContext: { labelKey: 'MUSIC_PLAYBACK_CONTEXT_PLAYLIST', labelParams: { name: 'My Playlist' } },
		});
		render(<GlobalPlayerControl />);
		expect(screen.getByText('MUSIC_PLAYBACK_FROM:MUSIC_PLAYBACK_CONTEXT_PLAYLIST:My Playlist')).toBeInTheDocument();
	});

	// Branch: playbackContext falsy => no context label
	it('does not show playback context label when playbackContext is undefined', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), playbackContext: undefined });
		render(<GlobalPlayerControl />);
	...
	it('uses currentTime when finite', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 65, duration: 200 });
		render(<GlobalPlayerControl />);
		expect(screen.getByText('1:05')).toBeInTheDocument();
	});

	it('uses 0 when currentTime is not finite (NaN)', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: NaN, duration: 100 });
		render(<GlobalPlayerControl />);
	...
	it('uses 0 when currentTime is Infinity', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: Infinity, duration: 100 });
		render(<GlobalPlayerControl />);
	...
	it('uses duration when finite and positive', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 0, duration: 185 });
		render(<GlobalPlayerControl />);
		expect(screen.getByText('3:05')).toBeInTheDocument();
	});

	it('uses 0 when duration is NaN', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 0, duration: NaN });
		render(<GlobalPlayerControl />);
		const timeTexts = screen.getAllByText('0:00');
	...
	it('uses 0 when duration is 0', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 0, duration: 0 });
		render(<GlobalPlayerControl />);
		const timeTexts = screen.getAllByText('0:00');
		expect(timeTexts.length).toBe(2);
	});
	...
	it('uses 0 when duration is negative', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 0, duration: -10 });
		render(<GlobalPlayerControl />);
	...
	it('sets slider max to actual duration when safeDuration > 0', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 0, duration: 200 });
		render(<GlobalPlayerControl />);
		const seekSlider = screen.getByLabelText('seek playback');
	...
	it('formats NaN time as 0:00', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: NaN, duration: NaN });
		render(<GlobalPlayerControl />);
		const timeTexts = screen.getAllByText('0:00');
		expect(timeTexts.length).toBe(2);
	});
	...
	it('formats valid time correctly', () => {
		mockUseGlobalMusic.mockReturnValue({ ...baseApi(), currentTime: 125, duration: 300 });
		render(<GlobalPlayerControl />);
	...
	it('shows playback context with context param', () => {
		mockUseGlobalMusic.mockReturnValue({
			...baseApi(),
			playbackContext: { labelKey: 'CONTEXT_KEY', labelParams: { context: 'Album X' } },
		});
		render(<GlobalPlayerControl />);
	...
	it('falls back to metadata.title when getMusicTitle is undefined', () => {
		...
		mockUseGlobalMusic.mockReturnValue({
			...baseApi(),
			currentTrack: { name: 'Fallback Name', metadata: { title: 'Fallback Title', artist: 'Art' } },
		});
		render(<GlobalPlayerControl />);
	...
	it('falls back to metadata.artist when getMusicArtist is undefined', () => {
		...
		mockUseGlobalMusic.mockReturnValue({
			...baseApi(),
			currentTrack: { name: 'Song', metadata: { title: 'T', artist: 'Fallback Artist' } },
		});
		render(<GlobalPlayerControl />);
