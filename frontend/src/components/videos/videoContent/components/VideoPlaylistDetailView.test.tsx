...
const createPlaylistItem = (id: number, videoId: number, orderIndex: number, overrides?: Partial<VideoPlaylistItemDto>): VideoPlaylistItemDto => ({
	id,
...
describe('VideoPlaylistDetailView', () => {
	...
	it('renders the hero cover image from cover_video_id', () => {
		...
		expect(heroImg).toHaveAttribute('src', 'http://localhost:8000/api/v1/files/video-thumbnail/10?width=1280&height=720');
	});
	...
	it('sorts items by order_index', () => {
		...
		const { container } = render(<VideoPlaylistDetailView {...defaultProps()} playlist={playlist} />);
	...
	it('disables remove buttons when isRemoving is true', () => {
		...
		const deleteButtons = screen.getAllByRole('button').filter((btn) =>
			btn.className.includes('iconBtnDanger'),
		);
	...
	it('calls onMoveItem with index and direction -1 for move up', () => {
		...
		const upButtons = screen.getAllByRole('button').filter((btn) =>
			btn.className.includes('iconBtn') && !btn.className.includes('Danger'),
		);
	...
	it('renders video thumbnails for each item', () => {
		...
		expect(images[1]).toHaveAttribute('src', 'http://localhost:8000/api/v1/files/video-thumbnail/10?width=320&height=180');
		expect(images[2]).toHaveAttribute('src', 'http://localhost:8000/api/v1/files/video-thumbnail/20?width=320&height=180');
	});
	...
	it('does not render hero image when no cover_video_id and no items', () => {
		const playlist = createPlaylist({ cover_video_id: null, items: [], item_count: 0 });
		render(<VideoPlaylistDetailView {...defaultProps()} playlist={playlist} />);
