...
import type {
	...
	VideoPlaylistDto,
} from '@/components/providers/videoContentProvider/videoContentProvider';
...
jest.mock('./VideoSeriesDetailView', () => ({
	__esModule: true,
	default: (props: any) => <div data-testid='series-detail'>{props.playlist?.classification}</div>,
}));
...
const createContext = (overrides: Partial<VideoContentContextData> = {}): VideoContentContextData => ({
	currentSection: 'home',
	playlists: [],
	allVideos: [],
	filteredVideos: [],
...
describe('VideoContentScreen', () => {
	...
	it.each(Object.entries(sectionTitleMap))(
		'renders the %s section grid',
		(section, titleKey) => {
