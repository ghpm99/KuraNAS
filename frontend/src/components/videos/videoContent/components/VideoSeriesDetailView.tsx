...
export default function VideoSeriesDetailView({ playlist, onBack, onOpenVideo }: VideoSeriesDetailViewProps) {
	const { t } = useI18n();
	const { groupedSeasons, completedCount, orderedItems, resumeItem } = useVideoPlaylistDetail(playlist);
	const [selectedSeasonKey, setSelectedSeasonKey] = useState('');
	const resolvedSeasonKey =
		groupedSeasons.find((group) => group.key === selectedSeasonKey)?.key ?? groupedSeasons[0]?.key ?? '';
					...
					<div className={styles.seasonSelector}>
							{groupedSeasons.map((group) => (
								<button
									key={group.key}
									type='button'
									className={group.key === resolvedSeasonKey ? styles.seasonChipActive : styles.seasonChip}
			...
			{visibleSeasons.map((group) => (
				<section key={group.key} className={styles.sectionBlock}>
					<div className={styles.sectionHeader}>
						...
						<p>{t('VIDEO_DETAIL_SEASON_DESCRIPTION', { count: String(group.items.length) })}</p>
					</div>
