    ...
    default: () => ({
        t: (key: string, params?: Record<string, string | number>) => {
            const map: Record<string, string> = {
                ...
                VIDEO_SECTION_CONTINUE_DESCRIPTION:
                    'Playlists e series onde voce parou recentemente.',
                VIDEO_SECTION_SERIES: 'Series',
                VIDEO_SECTION_SERIES_DESCRIPTION:
                    'Colecoes automaticas para series e animes com sequencia clara.',
                VIDEO_SECTION_MOVIES: 'Filmes',
                VIDEO_SECTION_MOVIES_DESCRIPTION:
                    'Filmes organizados como biblioteca dedicada, sem misturar com outros contextos.',
                VIDEO_SECTION_PERSONAL: 'Pessoais',
                VIDEO_SECTION_PERSONAL_DESCRIPTION:
                    'Gravacoes e videos pessoais agrupados com contexto proprio.',
                VIDEO_SECTION_CLIPS: 'Clipes',
                VIDEO_SECTION_CLIPS_DESCRIPTION:
                    'Clipes curtos e videos de programa em uma trilha rapida de consumo.',
                VIDEO_SECTION_FOLDERS: 'Pastas',
                VIDEO_SECTION_FOLDERS_DESCRIPTION:
                    'Entrada por pasta para quando a organizacao fisica importa mais que a classificacao.',
                ...
                VIDEO_HOME_RECENT_DESCRIPTION:
                    'Ultimos videos detectados no catalogo para abrir direto do dominio.',
                ...
                VIDEO_ALL: 'Todos',
                VIDEO_ALL_DESC:
                    'Todos os videos do sistema com acao rapida para adicionar a playlists.',
                ...
                VIDEO_DETAIL_COLLECTION_ITEMS_DESCRIPTION:
                    'Continue do ponto certo ou abra qualquer item do contexto.',
            ...
            if (key === 'VIDEO_DETAIL_SEASON_LABEL')
                return `Temporada ${params?.season ?? ''}`.trim();
            if (key === 'VIDEO_DETAIL_SEASON_DESCRIPTION')
                return `${params?.count ?? 0} episodios organizados em sequencia.`;
            return map[key] ?? key;
        },
    }),
}));
    ...
    items: [
        {
            ...
            video: {
                ...
                name: 'Home Clip.mp4',
                format: 'mp4',
                path: '/videos/home-clip.mp4',
                parent_path: '/videos/home',
                size: 10,
            },
        },
    ],
...
describe('components/videos/videoContent', () => {
    ...
    it('renders folders section and handles add/play/search actions', async () => {
        ...
        expect(
            await screen.findByText('Video adicionado a playlist com sucesso.')
        ).toBeInTheDocument();
    });
    ...
    it('renders series detail branch from route-based detail path', () => {
        ...
        expect(mockNavigate).toHaveBeenCalledWith(
            '/video/31?playlist=1&from=%2Fvideos%2Fseries%2Fplaylist-one',
            {
    ...
    it('renders folder detail branch and actions', () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('Nome de exibicao'), {
    ...
    it('renders context detail branch for clips', () => {
        ...
        expect(mockNavigate).toHaveBeenCalledWith(
            '/video/32?playlist=3&from=%2Fvideos%2Fclips%2Fclip-playlist',
            {
    ...
    it('handles add-to-playlist error', async () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: /Adicionar/i }));
        expect(
            await screen.findByText('Nao foi possivel adicionar o video a playlist.')
        ).toBeInTheDocument();
    });
});
