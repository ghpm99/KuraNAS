...
export default function useVideoPlayerScreen() {
	...
	const originLabels = useMemo(() => {
		...
		if (originPathname === appRoutes.favorites || originPathname === appRoutes.legacyFavorites) {
			return {
	...
	const playlistDetail = useMemo(() => (playlist ? buildVideoPlaylistDetail(playlist) : null), [playlist]);
	...
	const relatedItems = useMemo(() => {
		...
		const itemsAfterCurrent = currentIndex >= 0 ? orderedItems.slice(currentIndex + 1) : orderedItems;
	...
	const resumeLabel =
		playbackState && playbackState.current_time > 0 && !playbackState.completed
			? t('VIDEO_PLAYER_RESUME_POSITION', { time: formatPlaybackTime(playbackState.current_time) })
			: '';

	const metadataLine = [playlist?.name && playlist.name !== currentVideo?.name ? playlist.name : '', positionLabel, resumeLabel]
		.filter(Boolean)
		.join(' • ');

	const relatedTitle =
		playlistDetail?.hasEpisodeData || playlist?.classification === 'series' || playlist?.classification === 'anime'
	...
	return {
		...
		contextDescription: t('VIDEO_PLAYER_FROM_CONTEXT', { context: originLabels.context }),
		metadataLine,
		nextItem,
		relatedItems,
		relatedTitle,
