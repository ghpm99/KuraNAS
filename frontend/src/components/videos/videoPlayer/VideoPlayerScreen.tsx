			...
			<aside className={styles.contextPanel}>
				...
				{nextItem ? (
					<button
						...
						aria-label={t('VIDEO_PLAYER_OPEN_VIDEO', { name: nextItem.displayTitle || nextItem.video.name })}
					>
						<span className={styles.cardEyebrow}>{t('VIDEO_PLAYER_UP_NEXT')}</span>
						<strong className={styles.cardTitle}>{nextItem.displayTitle || nextItem.video.name}</strong>
						...
						{nextItem.progress_pct > 0 ? (
							<div className={styles.progressTrack} aria-hidden='true'>
								<div className={styles.progressFill} style={{ width: `${nextItem.progress_pct}%` }} />
							</div>
						) : null}
					</button>
				) : null}
						...
						<div className={styles.relatedList}>
							{relatedItems.map((item) => (
								<button
									...
									aria-label={t('VIDEO_PLAYER_OPEN_VIDEO', { name: item.displayTitle || item.video.name })}
								>
									<div className={styles.relatedBody}>
										<div className={styles.relatedHeaderRow}>
											{item.sequenceLabel ? <span className={styles.sequenceTag}>{item.sequenceLabel}</span> : null}
											<span className={styles.statusBadge}>{t(statusKeyMap[item.status])}</span>
										</div>
										<strong className={styles.relatedTitle}>{item.displayTitle || item.video.name}</strong>
										<span className={styles.relatedMeta}>{item.video.name}</span>
										{item.progress_pct > 0 ? (
											<div className={styles.progressTrack} aria-hidden='true'>
