	...
	items: [
		{
			...
			status: 'in_progress',
			progress_pct: 50,
			video: { id: 30, name: 'My.Show.S01E01.mkv', format: 'mkv', path: '', parent_path: '/library/my-show', size: 1 },
		},
		{
			id: 131,
			order_index: 1,
			...
			status: 'not_started',
			progress_pct: 0,
			video: { id: 31, name: 'My.Show.S01E02.mkv', format: 'mkv', path: '', parent_path: '/library/my-show', size: 1 },
		},
		{
			id: 132,
			order_index: 2,
			...
			status: 'completed',
			progress_pct: 100,
			video: { id: 32, name: 'My.Show.S01E03.mkv', format: 'mkv', path: '', parent_path: '/library/my-show', size: 1 },
		},
	],
	...overrides,
});
...
const buildPlayerState = (overrides?: Partial<any>) => ({
	...
	setDuration: jest.fn(),
	currentVideo: { id: 30, name: 'My.Show.S01E01.mkv', format: 'mkv', path: '', parent_path: '/library/my-show', size: 1 },
...
describe('components/videos/videoPlayer/useVideoPlayerScreen', () => {
	...
	it('keeps the route synced with the active video without restarting playback after replace navigation', () => {
		...
		state = buildPlayerState({
			currentVideo: { id: 31, name: 'My.Show.S01E02.mkv', format: 'mkv', path: '', parent_path: '/library/my-show', size: 1 },
			playbackState: {
					...
					items: [
						{
							...
							status: 'in_progress',
							progress_pct: 50,
							video: { id: 30, name: 'Movie.mkv', format: 'mkv', path: '', parent_path: '/library/movies', size: 1 },
						},
					],
				}),
			}),
	...
	it('covers route origin variants and generic fallbacks without a loaded playlist', () => {
		const scenarios = [
			...
			{ from: '/starred', expectedBadge: 'Favorites', expectedContext: 'From Favorites' },
			{ from: '/videos/movies', expectedBadge: 'Videos', expectedContext: 'From Movies' },
			{ from: '/external', expectedBadge: 'Videos', expectedContext: 'From Videos' },
		];
	...
	it('skips autoplay when the route id is missing and formats long resume durations', () => {
		...
		mockUseVideoPlayer.mockReturnValue(
			buildPlayerState({
				...
				currentVideo: { id: 30, name: 'Long Movie.mkv', format: 'mkv', path: '', parent_path: '/movies', size: 1 },
			}),
		);
