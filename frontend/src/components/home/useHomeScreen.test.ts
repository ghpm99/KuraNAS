...
const setupDefaultQueries = (overrides?: Partial<Record<
	'analytics' | 'favorites' | 'images' | 'videoCatalog' | 'videoPlayback' | 'playerState' | 'nowPlaying' | 'nowPlayingTracks',
...
describe('useHomeScreen', () => {
	...
	describe('null and empty data derivations', () => {
		...
		it('slices recent_files to 6 items', () => {
			const files = Array.from({ length: 10 }, (_, i) => ({ id: i, name: `file${i}` }));
			setupDefaultQueries({
		...
		it('slices favoriteItems to homeFavoritesLimit (6)', () => {
			const items = Array.from({ length: 10 }, (_, i) => ({ id: i, name: `fav${i}` }));
			setupDefaultQueries({
		...
		it('slices recentImages to homeImagesLimit (6)', () => {
			const items = Array.from({ length: 10 }, (_, i) => ({ id: i, name: `img${i}` }));
			setupDefaultQueries({
	...
	describe('videoContinueItems', () => {
		it('returns items from the "continue" section', () => {
			const items = [
				{ video: { id: 1, name: 'v1' }, progress_pct: 10, status: 'in_progress' },
				{ video: { id: 2, name: 'v2' }, progress_pct: 30, status: 'in_progress' },
			];
			setupDefaultQueries({
		...
		it('slices continue items to max 4', () => {
			const items = Array.from({ length: 8 }, (_, i) => ({
				video: { id: i, name: `v${i}` }, progress_pct: 10, status: 'in_progress',
			}));
			setupDefaultQueries({
				videoCatalog: buildQueryState({
		...
		it('returns null when session has no video_id', () => {
			setupDefaultQueries({
				videoPlayback: buildQueryState({
					playlist: { id: 1, items: [] },
					playback_state: { video_id: null, current_time: 0, duration: 0, playlist_id: null },
				}),
			});
		...
		it('returns null when video_id is 0 (falsy)', () => {
			setupDefaultQueries({
				videoPlayback: buildQueryState({
					...
					playback_state: { video_id: 0, current_time: 0, duration: 0, playlist_id: null },
				}),
			});
		...
		it('returns null when activeItem is not found in playlist items', () => {
			setupDefaultQueries({
				videoPlayback: buildQueryState({
					...
					playback_state: { video_id: 5, current_time: 30, duration: 120, playlist_id: 1 },
				}),
			});
		...
		it('returns video resume when session is valid', () => {
			setupDefaultQueries({
				videoPlayback: buildQueryState({
					...
					playback_state: { video_id: 5, current_time: 60, duration: 120, playlist_id: 10 },
				}),
			});
		...
		it('handles zero duration in video resume', () => {
			setupDefaultQueries({
				videoPlayback: buildQueryState({
					...
					playback_state: { video_id: 5, current_time: 0, duration: 0, playlist_id: null },
				}),
			});
	...
	describe('fallbackMusicTrack', () => {
		...
		it('returns null when current_file_id is set but track not found in nowPlayingTracks', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: 10 }),
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 100, name: 'other.mp3', size: 100, metadata: { duration: 60 } } },
					],
				}),
			});
		...
		it('returns fallback track when found in nowPlayingTracks', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: 30 }),
				nowPlaying: buildQueryState({ id: 7, track_count: 3 }),
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'song.mp3', size: 500, updated_at: '2026-01-01', metadata: { duration: 200, title: 'Song' } } },
					],
				}),
			});
	...
	describe('musicResume', () => {
		...
		it('uses currentTrack over fallback when currentTrack is available', () => {
			const track = { id: 10, name: 'Live.mp3', size: 1024, updated_at: '2026-03-01', metadata: { duration: 300, title: 'Live' } };
		...
		it('uses fallback track when currentTrack is undefined', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: 60 }),
				nowPlaying: buildQueryState({ id: 7, track_count: 4 }),
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'Track.mp3', size: 2048, updated_at: '2026-03-14', metadata: { duration: 240, title: 'Track', artist: 'Artist' } } },
					],
				}),
			});
		...
		it('uses playerState.current_position default 0 when fallback and no position', () => {
			setupDefaultQueries({
				...
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } } },
					],
				}),
			});
		...
		it('uses 0 when playerState.data is null for fallback progressSeconds', () => {
			...
			const track = { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } };
			mockedUseGlobalMusic.mockReturnValue({
				queue: [],
				currentTrack: undefined,
				currentTime: 0,
		...
		it('falls through to 0 when playerState.data exists but current_position is undefined', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: undefined }),
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } } },
					],
		...
		it('uses Math.max(duration, metadata.duration) when currentTrack is available', () => {
			const track = { id: 10, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 400 } };
			mockedUseGlobalMusic.mockReturnValue({
				queue: [track],
				currentTrack: track,
				currentTime: 50,
		...
		it('uses duration when greater than metadata.duration for currentTrack', () => {
			const track = { id: 10, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } };
			mockedUseGlobalMusic.mockReturnValue({
				queue: [track],
				currentTrack: track,
				currentTime: 50,
		...
		it('uses 0 for metadata.duration when currentTrack has no metadata', () => {
			const track = { id: 10, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: null };
			mockedUseGlobalMusic.mockReturnValue({
				queue: [track],
				currentTrack: track,
				currentTime: 50,
		...
		it('uses metadata.duration for fallback track (no currentTrack)', () => {
			setupDefaultQueries({
				...
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 180 } } },
					],
				}),
			});
		...
		it('uses 0 when fallback track has no metadata.duration', () => {
			setupDefaultQueries({
				...
				nowPlayingTracks: buildQueryState({
					items: [
						{ file: { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: null } },
					],
				}),
			});
	...
	describe('musicResume.queueCount', () => {
		const trackWithMeta = { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } };
		...
		it('uses queue.length when > 0', () => {
			...
			setupDefaultQueries({
				...
				nowPlayingTracks: buildQueryState({ items: [{ file: trackWithMeta }, { file: trackWithMeta }] }),
			});
		...
		it('uses nowPlayingQuery.track_count when queue is empty', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: 5 }),
		...
		it('uses nowPlayingTracks.items.length when queue empty and track_count is 0', () => {
			setupDefaultQueries({
				...
				nowPlayingTracks: buildQueryState({ items: [{ file: trackWithMeta }, { file: { id: 43 } }] }),
			});
		...
		it('falls through to 0 when queue empty, track_count falsy, and items.length is 0', () => {
			setupDefaultQueries({
				playerState: buildQueryState({ current_file_id: 42, current_position: 5 }),
				nowPlaying: buildQueryState({ id: 7, track_count: 0 }),
				nowPlayingTracks: buildQueryState({ items: [{ file: { id: 42, name: 'T.mp3', size: 100, updated_at: '2026-01-01', metadata: { duration: 100 } } }] }),
			});
	...
	describe('integration: full data scenario', () => {
		it('derives all fields correctly when all data is present', () => {
			const currentTrack = { id: 10, name: 'Now.mp3', size: 512, updated_at: '2026-03-15', metadata: { duration: 300, title: 'Now', artist: 'Band' } };
			mockedUseGlobalMusic.mockReturnValue({
			...
			setupDefaultQueries({
				...
				videoCatalog: buildQueryState({
					sections: [
						{ key: 'continue', title: 'C', items: [{ video: { id: 3 }, progress_pct: 45 }] },
					],
				}),
				videoPlayback: buildQueryState({
					playlist: { id: 10, items: [{ video: { id: 3, name: 'Ep3', parent_path: '/s' } }] },
					playback_state: { video_id: 3, current_time: 90, duration: 180, playlist_id: 10 },
				}),
				playerState: buildQueryState({ current_file_id: 10, current_position: 50 }),
				nowPlaying: buildQueryState({ id: 7, track_count: 5 }),
	...
	describe('query configuration', () => {
		...
		it('calls queryFn correctly for each query', async () => {
			...
			await expect(mockedUseQuery.mock.calls[3][0].queryFn()).resolves.toEqual({ sections: [] });
			...
			// nowPlaying queryFn
			await expect(mockedUseQuery.mock.calls[6][0].queryFn()).resolves.toEqual({ id: 7 });
			// nowPlayingTracks queryFn
			await expect(mockedUseQuery.mock.calls[7][0].queryFn()).resolves.toEqual({ items: [] });
		});
	});
});
