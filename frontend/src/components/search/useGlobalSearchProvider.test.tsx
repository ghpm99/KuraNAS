...
jest.mock('@/components/i18n/provider/i18nContext', () => ({
	__esModule: true,
	default: () => ({ t: (key: string, params?: Record<string, string>) => (params ? `${key}:${JSON.stringify(params)}` : key) }),
}));
...
const mockSearchData = {
	files: [{ id: 1, name: 'FILE', path: '/path/file', format: 'mp4', type: 1, parent_path: '/', size: 1 }],
	folders: [{ id: 2, name: 'Folder', path: '/folder' }],
	artists: [{ key: 'artist-1', artist: 'Artist', track_count: 2, album_count: 1 }],
	albums: [{ key: 'album-1', album: 'Album', artist: 'Artist', track_count: 5, year: '2020' }],
	playlists: [
		{ id: 3, name: 'Music Playlist', scope: 'music', count: 5, source_path: '', classification: 'personal', description: 'desc' },
		{ id: 4, name: 'Video Playlist', scope: 'video', count: 3, source_path: '/path', classification: 'series', description: 'folder' },
		{ id: 5, name: 'Custom Video Playlist', scope: 'video', count: 2, source_path: '', classification: 'movie', description: 'custom desc' },
	],
	videos: [{ id: 6, name: 'Video', path: '/video.mp4', format: 'video/mp4' }],
	images: [{ id: 7, name: 'Image', path: '/image.jpg', context: 'Library', category: 'folder' }],
};

let mockUseQueryReturn: { data: typeof mockSearchData | undefined; isFetching: boolean } = { data: undefined, isFetching: false };
...
describe('useGlobalSearchProvider', () => {
	...
	describe('search result mapping', () => {
		...
		it('maps artist results correctly', () => {
			...
			expect(mockNavigate).toHaveBeenCalledWith({ pathname: '/music/artists', search: '?artist=artist-1' });
		});
		...
		it('maps album results correctly', () => {
			...
			expect(mockNavigate).toHaveBeenCalledWith({ pathname: '/music/albums', search: '?album=album-1' });
		});
		...
		it('maps music playlist results and navigates to music playlists route', () => {
			...
			expect(mockNavigate).toHaveBeenCalledWith({ pathname: '/music/playlists', search: '?playlist=3' });
		});
		...
		it('maps video playlist with source_path (folder type) and navigates via getVideoDetailRoute', () => {
			...
			expect(mockGetVideoSectionForPlaylist).toHaveBeenCalledWith({ type: 'folder', classification: 'series' });
		...
		it('maps video playlist without source_path (custom type) and navigates via getVideoDetailRoute', () => {
			...
			expect(mockGetVideoSectionForPlaylist).toHaveBeenCalledWith({ type: 'custom', classification: 'movie' });
			expect(mockNavigate).toHaveBeenCalled();
		});
		...
		it('maps video results and navigates with state', () => {
			...
			expect(mockNavigate).toHaveBeenCalledWith('/video/6', { state: { from: '/files', playlistId: null } });
		});
		...
		it('uses category as meta when context is empty', () => {
			mockUseQueryReturn = {
				data: {
					...mockSearchData,
					images: [{ id: 8, name: 'NoCtx', path: '/img2.jpg', context: '', category: 'photos' }],
				},
				isFetching: false,
			};
			const { result } = renderHook(() => useGlobalSearchProvider());
	...
	describe('keyboard navigation (handleInputKeyDown)', () => {
		...
		it('ArrowDown wraps around to first item from last item', () => {
			...
			for (let i = 0; i < totalItems - 1; i++) {
				act(() => {
					result.current.handleInputKeyDown({ key: 'ArrowDown', preventDefault: jest.fn() } as any);
				});
			}
		...
		it('omits sections when their result arrays are empty', () => {
			mockUseQueryReturn = {
				data: {
					...
					playlists: [],
					videos: [],
					images: [{ id: 10, name: 'OnlyImage', path: '/only.jpg', context: 'ctx', category: 'cat' }],
				},
				isFetching: false,
			};
			const { result } = renderHook(() => useGlobalSearchProvider());
