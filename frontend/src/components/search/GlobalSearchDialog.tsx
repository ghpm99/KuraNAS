...
import type { SearchDialogItem, SearchDialogSection, SearchItemKind } from './useGlobalSearchProvider';
		...
		>
			<DialogContent className={styles.content}>
				<div className={styles.searchField}>
					...
					/>
					{isFetching ? <CircularProgress size={18} /> : <span className={styles.shortcut}>{shortcut}</span>}
				</div>
					...
					{sections.map((section) => (
						<div key={section.id} className={styles.section}>
							...
							{section.items.map((item) => {
								const itemClassName = item.id === activeItemId ? `${styles.item} ${styles.itemActive}` : styles.item;
								...
								return (
									...
									>
										<span className={styles.itemIcon}>{getItemIcon(item.kind)}</span>
