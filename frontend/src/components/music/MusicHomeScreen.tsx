            ...
            <div className={styles.heroGrid}>
                <section className={`${styles.panel} ${styles.heroPanel}`}>
                    ...
                    <p className={styles.panelDescription}>
                        {t('MUSIC_HOME_CONTINUE_DESCRIPTION')}
                    </p>

                    <div className={styles.trackMeta}>
                        <strong>
                            {hasQueue ? currentTrackTitle : t('MUSIC_HOME_QUEUE_EMPTY_STATE')}
                        </strong>
                        <span>
                            {hasQueue ? currentTrackArtist : t('MUSIC_HOME_QUEUE_EMPTY_HELP')}
                        </span>
                        <span className={styles.metricCaption}>
                            {t('MUSIC_HOME_QUEUE_COUNT', { count: String(queueCount) })}
                        </span>
                    </div>
                ...
                <section className={styles.panel}>
                    ...
                    <p className={styles.panelDescription}>
                        {status === 'pending'
                            ? t('MUSIC_HOME_LIBRARY_LOADING')
                            : t('MUSIC_HOME_LIBRARY_READY')}
                    </p>
                    <div className={styles.metricGrid}>
                        <div className={styles.metricItem}>
                            <strong className={styles.metricValue}>{totalTracks}</strong>
                            <span className={styles.metricCaption}>
                                {t('MUSIC_HOME_LIBRARY_TRACKS')}
                            </span>
                        </div>
                        <div className={styles.metricItem}>
                ...
                <div className={styles.sectionHeader}>
                    <div>
                        ...
                        <h2 className={styles.sectionTitle}>
                            {t('MUSIC_HOME_FEATURED_PLAYLISTS')}
                        </h2>
                    </div>
                    <Button
                        component={Link}
                        to={getMusicRoute('playlists')}
                        variant="text"
                        size="small"
                    >
                ...
                <div className={styles.cardGrid}>
                    {isLoadingPlaylists && featuredPlaylists.length === 0 ? (
                        ...
                        <div className={styles.emptyCard}>
                            {t('MUSIC_HOME_FEATURED_PLAYLISTS_EMPTY')}
                        </div>
                    ) : (
                        featuredPlaylists.map((playlist) => (
                            <article key={playlist.id} className={styles.sectionCard}>
                                <span className={styles.cardEyebrow}>
                                    {playlist.is_system
                                        ? t('MUSIC_HOME_SYSTEM_PLAYLIST')
                                        : t('MUSIC_PLAYLISTS')}
                                </span>
                                <h3 className={styles.cardTitle}>{playlist.name}</h3>
                                <p className={styles.cardDescription}>
                                    {playlist.description || t('MUSIC_PLAYLISTS_DESCRIPTION')}
                                </p>
                                <span className={styles.metricCaption}>
                                ...
                                <div className={styles.actions}>
                                    ...
                                    >
                                        {isActionPending(playlist.actionKey)
                                            ? t('LOADING')
            ...
            <section className={styles.section}>
                ...
                <div className={styles.cardGrid}>
                    {artistHighlights.length === 0 ? (
                        <div className={styles.emptyCard}>
                            {t('MUSIC_HOME_RECENT_ARTISTS_EMPTY')}
                        </div>
                    ) : (
                        artistHighlights.map((artist) => (
                            <article key={artist.artist} className={styles.sectionCard}>
                                ...
                                <p className={styles.cardDescription}>
                                    {artist.track_count} {t('MUSIC_TRACKS_COUNT')} ·{' '}
                                ...
                                <div className={styles.actions}>
                                    ...
                                    >
                                        {isActionPending(artist.actionKey)
                                            ? t('LOADING')
                                            : t('MUSIC_HOME_PLAY_NOW')}
                                    </Button>
                                    <Button
            ...
            <section className={styles.section}>
                ...
                <div className={styles.cardGrid}>
                    {albumHighlights.length === 0 ? (
                        <div className={styles.emptyCard}>
                            {t('MUSIC_HOME_RECENT_ALBUMS_EMPTY')}
                        </div>
                    ) : (
                        albumHighlights.map((album) => (
                            ...
                            >
                                ...
                                <div className={styles.actions}>
                                    ...
                                    >
                                        {isActionPending(album.actionKey)
                                            ? t('LOADING')
                                            : t('MUSIC_HOME_PLAY_NOW')}
                                    </Button>
                                    <Button
