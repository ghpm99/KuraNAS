...
export const buildMusicArtistHighlights = (tracks: IMusicData[], limit: number): MusicArtistHighlight[] => {
	...
	for (const track of tracks) {
		...
		existingArtist.latestTimestamp = Math.max(existingArtist.latestTimestamp, getTrackTimestamp(track));

		groupedArtists.set(artist, existingArtist);
	}

	return [...groupedArtists.values()]
		.sort((left, right) => right.latestTimestamp - left.latestTimestamp || left.artist.localeCompare(right.artist))
		.slice(0, limit)
...
export const buildMusicAlbumHighlights = (tracks: IMusicData[], limit: number): MusicAlbumHighlight[] => {
	...
	for (const track of tracks) {
		...
		existingAlbum.latestTimestamp = Math.max(existingAlbum.latestTimestamp, getTrackTimestamp(track));

		groupedAlbums.set(albumKey, existingAlbum);
	}

	return [...groupedAlbums.values()]
		.sort((left, right) => right.latestTimestamp - left.latestTimestamp || left.album.localeCompare(right.album))
		.slice(0, limit)
