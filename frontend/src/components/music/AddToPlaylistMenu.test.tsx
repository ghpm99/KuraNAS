...
describe('components/music/AddToPlaylistMenu', () => {
	...
	it('adds track to selected playlist', () => {
		...
		expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_TRACK_ADDED', { variant: 'success' });
	...
	it('creates new playlist and adds track', () => {
		...
		fireEvent.change(screen.getByLabelText('MUSIC_PLAYLIST_NAME'), { target: { value: 'Roadtrip' } });
		...
		return waitFor(() => {
			...
			expect(invalidateQueries).toHaveBeenCalled();
			expect(mockEnqueueSnackbar).toHaveBeenCalledWith('MUSIC_PLAYLIST_CREATED_ADDED', { variant: 'success' });
			expect(onClose).toHaveBeenCalled();
		});
	});
	...
	it('shows warning when add mutation fails and opens button wrapper', () => {
		mockUseMutation
			.mockImplementationOnce((options: any) => ({ mutate: () => options.onError?.(), isPending: false }))
			.mockImplementationOnce((options: any) => ({ mutate: () => options.onSuccess?.(), isPending: false }));
	...
	it('shows empty playlists entry and handles create failure', () => {
		...
		fireEvent.click(screen.getByText('MUSIC_NEW_PLAYLIST'));
		fireEvent.change(screen.getByLabelText('MUSIC_PLAYLIST_NAME'), { target: { value: 'Fails' } });
		fireEvent.click(screen.getByRole('button', { name: 'ACTION_CREATE_ADD' }));
