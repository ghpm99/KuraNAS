...
describe('VideoContentProvider', () => {
	...
	it('provides default state', async () => {
		const { result } = renderHook(() => useVideoContentProvider(), { wrapper: createWrapper() });
	...
	it('categorizes playlists by classification and type', async () => {
		mockGetVideoPlaylists.mockResolvedValue([
			...
			createPlaylist({ id: 4, name: 'Personal Vids', classification: 'personal' }),
			createPlaylist({ id: 5, name: 'Fun Clips', classification: 'clip' }),
			createPlaylist({ id: 6, name: 'Programs', classification: 'program' }),
			createPlaylist({ id: 7, name: 'Folder Vids', type: 'folder', classification: 'series' }),
		]);
	...
	it('sorts continuePlaylists by last_played_at and applies playback state cover', async () => {
		mockGetVideoPlaylists.mockResolvedValue([
			createPlaylist({ id: 1, name: 'Old Show', last_played_at: '2026-01-01T00:00:00Z' }),
			createPlaylist({ id: 2, name: 'New Show', last_played_at: '2026-03-01T00:00:00Z' }),
	...
	it('filters videos by search via query refetch', async () => {
		mockGetVideoLibraryFiles.mockResolvedValue({
			items: [
				createVideoFile({ id: 1, name: 'action.mp4', parent_path: '/movies', format: '.mp4' }),
				createVideoFile({ id: 2, name: 'comedy.mkv', parent_path: '/movies', format: '.mkv' }),
			],
			pagination: { page: 1, page_size: 60, has_next: false, has_prev: false },
		});
	...
	it('playVideo navigates with playlist id and from state', async () => {
		...
		expect(mockNavigate).toHaveBeenCalledWith(
			...
			expect.objectContaining({ state: expect.objectContaining({ playlistId: 7 }) }),
		);
	});
	...
	it('selectPlaylist navigates to playlist detail route', async () => {
		...
		expect(mockNavigate).toHaveBeenCalledWith(expect.stringContaining('/videos/series/cool-series'));
	});
	...
	it('selectedPlaylistSummary and detail load when URL has slug', async () => {
		...
		mockGetVideoPlaylistById.mockResolvedValue(
			createPlaylist({
				...
				name: 'Action Movies',
				items: [
					{ id: 1, order_index: 0, source_kind: 'auto', video: createVideoFile({ id: 50 }), status: 'not_started', progress_pct: 0 },
					{ id: 2, order_index: 1, source_kind: 'auto', video: createVideoFile({ id: 51 }), status: 'not_started', progress_pct: 0 },
				],
			}),
		);
		...
		mockGetVideoPlaylistById.mockResolvedValue(
			createPlaylist({
				...
				items: [
					{ id: 1, order_index: 0, source_kind: 'auto', video: createVideoFile({ id: 50 }), status: 'not_started', progress_pct: 0 },
					{ id: 2, order_index: 1, source_kind: 'auto', video: createVideoFile({ id: 51 }), status: 'not_started', progress_pct: 0 },
					{ id: 3, order_index: 2, source_kind: 'auto', video: createVideoFile({ id: 52 }), status: 'not_started', progress_pct: 0 },
				],
			}),
		);
	...
	it('openPlaylistVideo navigates with playlist id', async () => {
		...
		expect(mockNavigate).toHaveBeenCalledWith(
			expect.stringContaining('/video/42'),
			expect.objectContaining({ state: expect.objectContaining({ playlistId: 5 }) }),
		);
	});
			...
			sections: [
				{
					key: 'recent',
					title: 'Recent',
					items: [{ video: createVideoFile({ id: 1 }), status: 'not_started', progress_pct: 0 }],
				},
			],
		});
	...
	it('continuePlaylists handles playlists where one has null last_played_at time gracefully', async () => {
		mockGetVideoPlaylists.mockResolvedValue([
			createPlaylist({ id: 1, name: 'Show A', last_played_at: '2026-01-15T00:00:00Z' }),
			createPlaylist({ id: 2, name: 'Show B', last_played_at: '2026-02-15T00:00:00Z' }),
		]);
		mockGetVideoPlaybackState.mockResolvedValue(null);
	...
	it('renameSelectedPlaylist calls mutation with valid name', async () => {
		mockGetVideoPlaylists.mockResolvedValue([
			createPlaylist({ id: 5, name: 'My Playlist', classification: 'personal' }),
		]);
		...
		await waitFor(() => expect(mockUpdateVideoPlaylistName).toHaveBeenCalledWith(5, 'New Name'));
	});
	...
	it('playVideo without playlist id navigates without playlist param', async () => {
		...
		expect(mockNavigate).toHaveBeenCalledWith(
			expect.stringContaining('/video/42'),
			expect.objectContaining({ state: expect.objectContaining({ playlistId: null }) }),
		);
	});
	...
	it('selectPlaylist resolves section from home when on home route', async () => {
		const folderPlaylist = createPlaylist({ id: 1, name: 'Folder Vids', type: 'folder', classification: 'series' });
