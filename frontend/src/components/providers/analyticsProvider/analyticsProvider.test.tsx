...
const mockOverview: AnalyticsOverview = {
	...
	library: { categorized_media: 0, audio_with_metadata: 0, video_with_metadata: 0, image_with_metadata: 0, image_classified: 0 },
	processing: { metadata_pending: 0, metadata_failed: 0, thumbnail_pending: 0, thumbnail_failed: 0 },
	health: { status: 'ok', last_scan_at: '', last_scan_seconds: 0, indexed_files: 0, errors_last_24h: 0, recent_errors: [] },
};
...
function ConsumerComponent() {
	...
	return (
		<div>
			...
			<button data-testid='refresh' onClick={() => void ctx.refresh()}>Refresh</button>
			<button data-testid='set-period' onClick={() => ctx.setPeriod('30d')}>Set 30d</button>
		</div>
	);
}
