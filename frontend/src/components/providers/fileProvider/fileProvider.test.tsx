...
// helpers
const createNode = (id: number, overrides?: Partial<FileData>, children?: FileData[]): FileData => ({
	id,
	name: `node-${id}`,
	path: `/node-${id}`,
	parent_path: '/',
...
const makePaginationResponse = (items: FileData[], hasNext = false, page = 1): PaginationResponse => ({
	items,
...
describe('useFile', () => {
	it('throws when used outside of FileProvider', () => {
		...
		expect(() => render(<ErrorComponent />)).toThrow('useFile must be used within a FileProvider');
	});
});
...
describe('FileProvider', () => {
	...
	it('resolves selected item from URL path', async () => {
		...
		const childItems = [createNode(20, { name: 'readme.txt', path: '/docs/readme.txt', type: 2 })];
	...
	it('expandedItems contains trail ids when selected item is in tree', async () => {
		...
		mockPathname = '/files';
		const rootTree = [createNode(10, { name: 'docs', path: '/docs', type: 1 }, [createNode(20, { name: 'sub', path: '/docs/sub', type: 1 })])];
		...
		mockGetFileByPath.mockResolvedValue(createNode(20, { name: 'sub', path: '/docs/sub', type: 1 }));
		mockGetFilesTree.mockResolvedValue(makePaginationResponse([]));
	...
	it('fetches recent access data when selected item is a file', async () => {
		...
		const resolved = createNode(10, { name: 'readme.txt', path: '/readme.txt', type: 2 });
		mockGetFileByPath.mockResolvedValue(resolved);
		mockGetFilesTree.mockResolvedValue(makePaginationResponse([]));
		const accessData = [{ id: 1, ip_address: '127.0.0.1', file_id: 10, accessed_at: '2026-01-01' }];
	...
	it('supports pagination when hasNext is true', async () => {
		...
		mockGetFilesTree.mockResolvedValueOnce(
			makePaginationResponse(firstPage, true, 1),
		);
