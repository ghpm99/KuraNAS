...
export default function useMusicStateSync(deps: SyncDeps) {
	...
	const syncState = useCallback((overrides?: SyncOverrides) => {
		...
		syncTimeoutRef.current = setTimeout(() => {
			...
			updatePlayerState({
				playlist_id: overrides?.playlistId !== undefined ? overrides.playlistId : (d.playbackContext?.playlistId ?? null),
				current_file_id: overrides?.fileId !== undefined ? overrides.fileId : (d.getCurrentTrackId() ?? null),
				current_position: overrides?.position !== undefined ? overrides.position : d.getCurrentTime(),
