...
const ActivityDiaryProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [message, setMessage] = useState<{ text: string; type: messageType } | undefined>(
        undefined
    );
    ...
    const duplicateDiaryMutation = useMutation({
        mutationFn: (diaryId: number): Promise<ActivityDiaryData> =>
            duplicateActivityDiaryEntry(diaryId),
        onSuccess: () => {
    ...
    return (
        <ActivityDiaryContextProvider value={contextValue}>{children}</ActivityDiaryContextProvider>
    );
};
