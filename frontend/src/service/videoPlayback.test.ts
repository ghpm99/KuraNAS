...
describe('service/videoPlayback', () => {
	...
	it('gets and updates playback state', async () => {
		mockedApi.get.mockResolvedValue({ data: { playback_state: { current_time: 12 } } });
		mockedApi.put.mockResolvedValue({ data: { current_time: 25 } });

		const state = await getVideoPlaybackState();
		const updated = await updateVideoPlaybackState({ current_time: 25, is_paused: true });
	...
	it('gets catalog and playlists with default and explicit params', async () => {
		...
		expect(mockedApi.get).toHaveBeenNthCalledWith(1, '/video/catalog/home', { params: { limit: 24 } });
		expect(mockedApi.get).toHaveBeenNthCalledWith(2, '/video/catalog/home', { params: { limit: 50 } });
	...
	it('updates playlist visibility and order, add and remove video, and rename playlist', async () => {
		...
		expect(mockedApi.put).toHaveBeenNthCalledWith(1, '/video/playlists/3/hidden', { hidden: true });
		expect(mockedApi.post).toHaveBeenCalledWith('/video/playlists/3/videos', { video_id: 101 });
		...
		expect(mockedApi.put).toHaveBeenNthCalledWith(3, '/video/playlists/3', { name: 'Minha lista' });
	});
