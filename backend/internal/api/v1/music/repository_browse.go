...
func NewAudioMetadataRepository(db *database.DbContext) *AudioMetadataRepository {
	return &AudioMetadataRepository{Db: db}
}

func (r *AudioMetadataRepository) GetDbContext() *database.DbContext {
	return r.Db
}

func (r *AudioMetadataRepository) GetAudioMetadataByID(id int) (AudioMetadataModel, error) {
...
func (r *AudioMetadataRepository) UpsertAudioMetadata(tx *sql.Tx, metadata AudioMetadataModel) (AudioMetadataModel, error) {
	...
	if err != nil {
		log.Println("Error scanning audio metadata:", err)
		return metadata, err
	}
...
func (r *Repository) GetMusic(page int, pageSize int) (utils.PaginationResponse[files.FileModel], error) {
	...
	err := r.DbContext.QueryTx(func(tx *sql.Tx) error {
		rows, err := tx.Query(queries.GetMusicQuery, args...)
		if err != nil {
			return err
		}
		defer rows.Close()
	...
	if err != nil {
		return paginationResponse, fmt.Errorf("failed to query music files: %w", err)
	}
...
func (r *Repository) GetMusicByArtist(artist string, page int, pageSize int) (utils.PaginationResponse[files.FileModel], error) {
	paginationResponse := utils.PaginationResponse[files.FileModel]{
		Items: []files.FileModel{},
		Pagination: utils.Pagination{
			Page:     page,
...
func (r *Repository) GetMusicByAlbum(album string, page int, pageSize int) (utils.PaginationResponse[files.FileModel], error) {
	paginationResponse := utils.PaginationResponse[files.FileModel]{
		Items: []files.FileModel{},
		Pagination: utils.Pagination{
			Page:     page,
...
func (r *Repository) GetMusicByGenre(genre string, page int, pageSize int) (utils.PaginationResponse[files.FileModel], error) {
	paginationResponse := utils.PaginationResponse[files.FileModel]{
		Items: []files.FileModel{},
		Pagination: utils.Pagination{
			Page:     page,
