...
func RegisterFilesRoutes(router *gin.RouterGroup, context *AppContext) {
	...
	files.POST("/copy", context.Files.Handler.CopyPathHandler)
	files.POST("/rename", context.Files.Handler.RenamePathHandler)
	files.DELETE("/path", context.Files.Handler.DeletePathHandler)
