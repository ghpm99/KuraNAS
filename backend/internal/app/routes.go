...
func RegisterRoutes(router *gin.Engine, context *AppContext) {
	...
	RegisterAutoShutdownRoutes(routesV1, context)
...
func RegisterEmailRoutes(router *gin.RouterGroup, context *AppContext) {
	...
	if context == nil || context.Email == nil || context.Email.Handler == nil {
		...
		group.Any("/messages/*rest", email.DisabledHandler)
		group.Any("/settings/*rest", email.DisabledHandler)
		return
	}
	...
	group.GET("/messages/:id/summary", handler.GetMessageSummaryHandler)
	group.GET("/settings/provider", handler.GetProviderHandler)
	group.PUT("/settings/provider", handler.SetProviderHandler)
}
...
func RegisterBackupRoutes(router *gin.RouterGroup, context *AppContext) {
	if context == nil || context.Backup == nil || context.Backup.Handler == nil {
		return
	}

	group := router.Group("/backup")
	group.GET("/settings", context.Backup.Handler.GetSettingsHandler)
...
func RegisterAutoShutdownRoutes(router *gin.RouterGroup, context *AppContext) {
	if context == nil || context.AutoShutdown == nil || context.AutoShutdown.Handler == nil {
		return
	}

	group := router.Group("/auto-shutdown")
	group.GET("/settings", context.AutoShutdown.Handler.GetSettingsHandler)
	group.PUT("/settings", context.AutoShutdown.Handler.UpdateSettingsHandler)
	group.GET("/suggested-time", context.AutoShutdown.Handler.GetSuggestedTimeHandler)
}
